...
### selected 19 features and demographic features for two+ observations
feature_list = ['AGE_new', 'Sex_n', 'BMI', 'ALC', 'SMOKING', \
                ...
                'PLT_mean', 'PLT_median', 'PLT_std' ]

### random 4 features for comparison
# feature_list = ['AGE_new', 'Sex_n', 'BMI', 'ALC', 'SMOKING', 'CREA_mean', 'CREA_median', 'CREA_std',\
#                 'BILI_mean', 'BILI_median', 'BILI_std','PROT_mean', 'PROT_median', 'PROT_std',\
...
## store for each fold, then take average
...
jaccard_values = np.zeros(5,); prec_values = np.zeros(5,)

for idx in range(5):
    print(f'~~~~~~ Working with fold: {idx}~~~~~')
    fname = "~/Desktop/data/data_two+/k-fold/summary_nineteen_feature_two+_top_cancers_train_fold_"+str(idx)+"_year_window_3.csv"
    data3_tr = pd.read_csv(fname)
    fname = "~/Desktop/data/data_two+/k-fold/summary_nineteen_feature_two+_top_cancers_test_fold_"+str(idx)+"_year_window_3.csv"
    ...
    auc_values[idx] = auc
    TP_values[idx] = tp; FN_values[idx] = fn
    TN_values[idx] = tn; FP_values[idx] = fp
    sen_values[idx] = sen; spe_values[idx] = spe
    jaccard_values[idx] = jaccard; prec_values[idx] = pre
    
    feature_list.remove('Reference Key')

## get average of 5-fold
avg_auc = np.average(auc_values)
avg_tp = np.average(TP_values); avg_fn = np.average(FN_values)
avg_tn = np.average(TN_values); avg_fp = np.average(FP_values)
avg_sen = np.average(sen_values); avg_spe = np.average(spe_values);
avg_jaccard = np.average(jaccard_values); avg_prec = np.average(prec_values)
...
print(f'F1-score: {avg_f1}\tJaccard Index: {avg_jaccard}\t \tROC-AUC: {avg_auc}')
