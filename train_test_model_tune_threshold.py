...
### get optimal threshold rather than the default 0.5
def getOptimalThresholdforSensitivity(avg_prob, true_label):
    fpr, tpr, thresholds = roc_curve(true_label, avg_prob)

    # maximize sensitivity - false positive rate
    optimal_idx = (tpr - fpr).argmax()
    optimal_threshold = thresholds[optimal_idx]

    return optimal_threshold

def calculateAvgPerformanceWithThre(avg_prob, ref33, true_label, threshold):
    ...
    for i in range(len(ref33)):
        pred_label = 1 if avg_prob[i] > threshold else 0
        if true_label[i] ==1:
            if pred_label == true_label[i]:
                tp += 1; 
            else:
    ...
    return tp, fn, fp, tn, tp/(tp+fn), tn/(tn+fp), prec, tp/(tp+fp+fn), (tp+tn)/(tp+tn+fn+fp), auc

def getAvgProbability(data3_ts, data2_ts, data1_ts, data0_ts, feature_list, clf3, clf2, clf1, clf0):
    ...
    return ref33, avg_prob, true_label ### return the ref keys of selected samples across four window and avg prob
#############################################


model = sys.argv[1] ### get model name
### selected 16 feature and demographic features
...
data2 = pd.read_csv("~/Desktop/data/data/summary_features_five+_year_window_2.csv")
data1 = pd.read_csv("~/Desktop/data/data/summary_features_five+_year_window_1.csv")
data0 = pd.read_csv("~/Desktop/data/data/summary_features_five+_year_window_0.csv")


for idx in range(5):
    print(f'~~~~~~ Working with fold: {idx}~~~~~')
    fname = "~/Desktop/data/data/k-fold/summary_fifteen_feature_five+_top_cancers_train_fold_"+str(idx)+"_year_window_3.csv"
    data3_tr = pd.read_csv(fname)
    fname = "~/Desktop/data/data/k-fold/summary_fifteen_feature_five+_top_cancers_test_fold_"+str(idx)+"_year_window_3.csv"
    ...
    y3_tr = data3_tr['CANCER']
    data3_tr1, data3_val, y3_tr1, y3_val = train_test_split( data3_tr, y3_tr, test_size=0.1, stratify=y3_tr,  random_state=19)

    y2_tr = data2_tr['CANCER']
    data2_tr1, data2_val, y2_tr1, y2_val = train_test_split( data2_tr, y2_tr, test_size=0.1, stratify=y2_tr,  random_state=19)

    y1_tr = data1_tr['CANCER']
    data1_tr1, data1_val, y1_tr1, y1_val = train_test_split( data1_tr, y1_tr, test_size=0.1, stratify=y1_tr,  random_state=19)

    y0_tr = data0_tr['CANCER']
    data0_tr1, data0_val, y0_tr1, y0_val = train_test_split( data0_tr, y0_tr, test_size=0.1, stratify=y0_tr,  random_state=19)
    ### train a model
    clf3 = train_model(model, data3_tr1, feature_list, year_window=3)
    clf2 = train_model(model, data2_tr1, feature_list, year_window=2)
    clf1 = train_model(model, data1_tr1, feature_list, year_window=1)
    clf0 = train_model(model, data0_tr1, feature_list, year_window=0)
    print("~~~~ Model training completed!~~~~")
    ### use train data to get optimal decision threshold
    ref33_val, avg_prob_val, true_label_val = getAvgProbability(data3_val, data2_val, data1_val, data0_val, feature_list, clf3, clf2, clf1, clf0)
    ### get optimal threshold
    opt_threshold = getOptimalThresholdforSensitivity(avg_prob_val, true_label_val)
    ...
    ref33, avg_prob_ts, true_label_ts = getAvgProbability(data3_ts, data2_ts, data1_ts, data0_ts, feature_list, clf3, clf2, clf1, clf0)

    # calculate performance using avg prob
    tp, fn, fp, tn, sen, spe, pre, jaccard, accu, auc = calculateAvgPerformanceWithThre(avg_prob_ts, ref33, true_label_ts, opt_threshold)
